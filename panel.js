...
// VK Mini Apps передают подписанные launch params в query string.
// Сервер проверяет sign через защищённый ключ приложения.
function verifyMiniLaunch(req){
  const q=req.query||{};
  const appId=String(process.env.VK_CLIENT_ID||'');
  if(!appId || String(q.vk_app_id||'')!==appId || !q.vk_user_id || !q.sign) return null;
  const crypto=require('crypto');
  const params={};
  for(const [k,v] of Object.entries(q)) if(k.startsWith('vk_')) params[k]=String(v);
  const check=Object.keys(params).sort().map(k=>encodeURIComponent(k)+'='+encodeURIComponent(params[k])).join('&');
  const secret=String(process.env.VK_CLIENT_SECRET||'');
  if(!secret) return null;
  const expected=crypto.createHmac('sha256',secret).update(check).digest('base64').replace(/\+/g,'-').replace(/\//g,'_').replace(/=+$/,'');
  if(!crypto.timingSafeEqual(Buffer.from(expected),Buffer.from(String(q.sign)))) return null;
  const ts=Number(q.vk_ts||0);
  if(ts && Math.abs(Date.now()/1000-ts)>86400) return null;
  return String(q.vk_user_id);
}

async function establishMiniSession(req,res,vkId){
  ...
  if(!admin){
    ...
    if(bootstrap && bootstrap===vkId){
      ...
      await db.run(`INSERT INTO web_admins (username,password_hash,role_id,vk_id,discord_id,nickname,position,appointment_reason,vk_avatar,admin_level) VALUES (?,?,?,?,?,?,?,?,?,?)`,[`vk_${vkId}`,'',roleId,vkId,process.env.VK_BOOTSTRAP_DISCORD_ID||'',`VK ${vkId}`,'Владелец','Первичная настройка панели','',8]);
  ...
  req.session.vkId=vkId;
  req.session.isAdmin=true;
  return true;
}

app.get('/auth/vk', (req,res)=>{
  // Старый OAuth URL больше не используется: панель теперь работает как VK Mini App.
  res.redirect('/');
});

app.get('/auth/vk/callback', (req,res)=>res.status(410).send('<h2 style="font-family:system-ui;text-align:center;margin-top:15vh">Этот адрес больше не используется.</h2><p style="text-align:center">Откройте SerkovTools через VK Mini App.</p>'));

app.get('/auth/vk/mini', async (req,res)=>{
  try{
    const vkId=verifyMiniLaunch(req);
    if(!vkId) return res.status(401).send('<h2 style="font-family:system-ui;text-align:center;margin-top:15vh">Не удалось проверить VK.</h2><p style="text-align:center">Откройте панель именно внутри VK Mini App.</p>');
    const ok=await establishMiniSession(req,res,vkId);
    if(!ok) return res.status(403).send(`<!doctype html><html lang="ru"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><body style="margin:0;min-height:100vh;display:grid;place-items:center;background:#080611;color:#fff;font-family:system-ui"><div style="max-width:460px;padding:28px;text-align:center;border:1px solid #3b2b5e;border-radius:24px;background:#120c20"><h2>Доступ не предоставлен</h2><p style="color:#aaa">Ваш VK ID <b>${vkId}</b> ещё не добавлен в администрацию. Попросите администратора создать профиль.</p></div></body></html>`);
    res.redirect('/');
  }catch(e){ console.error('[VK MINI AUTH]',e); res.status(500).send('Ошибка авторизации VK Mini App.'); }
});

app.get('/login',(req,res)=>res.send(`<!doctype html><html lang="ru"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>SerkovTools — VK</title><style>body{margin:0;min-height:100vh;display:grid;place-items:center;background:radial-gradient(circle at 20% 10%,#40208055,transparent 35%),#080611;color:#fff;font-family:Inter,system-ui}.card{width:min(420px,calc(100% - 32px));padding:34px;border:1px solid #ffffff18;border-radius:28px;background:linear-gradient(145deg,#1d1230e8,#0b0813ee);box-shadow:0 30px 90px #0008;text-align:center}.logo{font-size:42px;margin-bottom:12px}.muted{color:#a99fba;line-height:1.6}.vk{display:block;margin-top:24px;padding:15px;border-radius:16px;text-decoration:none;color:#fff;font-weight:800;background:linear-gradient(135deg,#0077ff,#4aa3ff);box-shadow:0 14px 34px #0878ff33}.foot{margin-top:16px;color:#756b8e;font-size:12px}</style></head><body><main class="card"><div class="logo">⚡</div><h1>SerkovTools</h1><p class="muted">Закрытая панель администрации.<br>Вход выполняется только через VK Mini App.</p><a class="vk" href="/">Открыть панель</a><div class="foot">Откройте приложение из VK, чтобы передать подписанные параметры пользователя.</div></main></body></html>`));
...
async function requireAdmin(req,res,next){ if(req.session?.isAdmin && req.session.adminId)return next(); const vkId=verifyMiniLaunch(req); if(vkId){ const ok=await establishMiniSession(req,res,vkId); if(ok)return next(); return res.status(403).send('Доступ к панели не предоставлен.'); } return res.redirect('/login'); }
