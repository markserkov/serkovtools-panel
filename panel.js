...
app.post('/bridge/poll', async (req, res) => {
    ...
    if (Array.isArray(body.adminProfiles)) {
        for (const a of body.adminProfiles) {
            if (!a) continue;
            if(a.telegram_id) await db.run(`UPDATE web_admins SET nickname=COALESCE(NULLIF(?,'') ,nickname), telegram_avatar=COALESCE(NULLIF(?,'') ,telegram_avatar) WHERE telegram_id=?`, [String(a.nickname||''), String(a.vk_avatar||''), String(a.telegram_id)]);
            else if(a.vk_id) await db.run(`UPDATE web_admins SET nickname=COALESCE(NULLIF(?,'') ,nickname), vk_avatar=COALESCE(NULLIF(?,'') ,vk_avatar) WHERE vk_id=?`, [String(a.nickname||''), String(a.vk_avatar||''), String(a.vk_id)]);
        }
    }
...
for (const [name,type] of [['vk_id','TEXT'],['telegram_id','TEXT'],['telegram_username','TEXT'],['telegram_avatar','TEXT'],['discord_id','TEXT'],['nickname','TEXT'],['position','TEXT'],['appointment_reason','TEXT'],['vk_avatar','TEXT'],['admin_level','INTEGER']]) {
...
await db.exec(`CREATE UNIQUE INDEX IF NOT EXISTS idx_web_admins_telegram_id ON web_admins(telegram_id) WHERE telegram_id IS NOT NULL AND telegram_id <> ''`);
...
// ---------- Telegram Login (OIDC) authentication ----------
...
const TELEGRAM_REDIRECT_URI = String(process.env.TELEGRAM_REDIRECT_URI || 'https://serkovtools-panel.onrender.com/auth/telegram/callback');
let telegramJWKS = null;
let telegramJWKSAt = 0;

function b64url(buf){ return Buffer.from(buf).toString('base64').replace(/\+/g,'-').replace(/\//g,'_').replace(/=+$/,''); }
function randomString(n=32){ return b64url(crypto.randomBytes(n)); }
function pkceChallenge(verifier){ return b64url(crypto.createHash('sha256').update(verifier).digest()); }
function decodeJwtPart(part){ return JSON.parse(Buffer.from(part,'base64url').toString('utf8')); }
async function getTelegramJWKS(){
  if(telegramJWKS && Date.now()-telegramJWKSAt<6*60*60*1000) return telegramJWKS;
  const r=await fetch('https://oauth.telegram.org/.well-known/jwks.json');
  if(!r.ok) throw new Error('Telegram JWKS HTTP '+r.status);
  telegramJWKS=await r.json(); telegramJWKSAt=Date.now(); return telegramJWKS;
}
async function verifyTelegramIdToken(token, nonce){
  const parts=String(token||'').split('.'); if(parts.length!==3) throw new Error('Invalid Telegram ID token');
  ...
  if(String(payload.aud)!==TELEGRAM_CLIENT_ID) throw new Error('Invalid Telegram audience');
  if(payload.exp && Number(payload.exp)<Math.floor(Date.now()/1000)) throw new Error('Telegram token expired');
  if(nonce && payload.nonce!==nonce) throw new Error('Invalid Telegram nonce');
  ...
  const ok=crypto.verify(header.alg==='ES256'?'sha256':'RSA-SHA256',Buffer.from(parts[0]+'.'+parts[1]),key,Buffer.from(parts[2],'base64url'));
  if(!ok) throw new Error('Invalid Telegram token signature');
  return payload;
}
async function establishTelegramSession(req,res,tg){
  const telegramId=String(tg.sub||tg.id||''); if(!telegramId) return false;
  let admin=await db.get(`SELECT a.*, r.name AS role_name, r.level, r.permissions FROM web_admins a LEFT JOIN web_roles r ON a.role_id=r.id WHERE a.telegram_id=?`,[telegramId]);
  if(!admin){
    const bootstrap=String(process.env.TELEGRAM_BOOTSTRAP_ID||'');
    if(bootstrap && bootstrap===telegramId){
      ...
      await db.run(`INSERT INTO web_admins (username,password_hash,role_id,telegram_id,telegram_username,telegram_avatar,discord_id,nickname,position,appointment_reason,admin_level) VALUES (?,?,?,?,?,?,?,?,?,?,?)`,[
        `tg_${telegramId}`,'',roleId,telegramId,String(tg.preferred_username||''),String(tg.picture||''),process.env.TELEGRAM_BOOTSTRAP_DISCORD_ID||'',String(tg.name||tg.preferred_username||('TG '+telegramId)),'Владелец','Первичная настройка панели',8
      ]);
      admin=await db.get(`SELECT a.*, r.name AS role_name, r.level, r.permissions FROM web_admins a LEFT JOIN web_roles r ON a.role_id=r.id WHERE a.telegram_id=?`,[telegramId]);
    }
  }
  if(!admin) return false;
  await db.run(`UPDATE web_admins SET nickname=COALESCE(NULLIF(?,'') ,nickname), telegram_username=COALESCE(NULLIF(?,'') ,telegram_username), telegram_avatar=COALESCE(NULLIF(?,'') ,telegram_avatar) WHERE id=?`,[String(tg.name||tg.preferred_username||''),String(tg.preferred_username||''),String(tg.picture||''),admin.id]);
  req.session.adminId=admin.id; req.session.telegramId=telegramId; req.session.isAdmin=true;
  return true;
}
app.get('/auth/telegram',(req,res)=>{
  if(!TELEGRAM_CLIENT_ID || !TELEGRAM_CLIENT_SECRET) return res.status(500).send('Telegram Login не настроен на Render.');
  const state=randomString(24), verifier=randomString(48), nonce=randomString(24);
  req.session.telegramOAuth={state,verifier,nonce,createdAt:Date.now()};
  const u=new URL('https://oauth.telegram.org/auth');
  u.searchParams.set('client_id',TELEGRAM_CLIENT_ID); u.searchParams.set('bot_id',TELEGRAM_CLIENT_ID); u.searchParams.set('redirect_uri',TELEGRAM_REDIRECT_URI); u.searchParams.set('response_type','code');
  u.searchParams.set('scope','openid profile'); u.searchParams.set('state',state); u.searchParams.set('nonce',nonce); u.searchParams.set('code_challenge',pkceChallenge(verifier)); u.searchParams.set('code_challenge_method','S256');
  res.redirect(u.toString());
});
app.get('/auth/telegram/callback',async(req,res)=>{
  try{
    const o=req.session.telegramOAuth; if(!o || !req.query.code || req.query.state!==o.state) return res.status(400).send('Ошибка авторизации Telegram: неверный state.');
    if(Date.now()-o.createdAt>10*60*1000) return res.status(400).send('Сессия авторизации Telegram истекла.');
    const basic=Buffer.from(`${TELEGRAM_CLIENT_ID}:${TELEGRAM_CLIENT_SECRET}`).toString('base64');
    const body=new URLSearchParams({grant_type:'authorization_code',code:String(req.query.code),redirect_uri:TELEGRAM_REDIRECT_URI,client_id:TELEGRAM_CLIENT_ID,code_verifier:o.verifier});
    const tr=await fetch('https://oauth.telegram.org/token',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded','Authorization':`Basic ${basic}`},body});
    const td=await tr.json(); if(!tr.ok || !td.id_token) throw new Error(td.error_description||td.error||'Не удалось получить Telegram ID token');
    const tg=await verifyTelegramIdToken(td.id_token,o.nonce); delete req.session.telegramOAuth;
    const ok=await establishTelegramSession(req,res,tg); if(!ok) return res.status(403).send(`<h2 style="font-family:system-ui;text-align:center;margin-top:15vh">Доступ не предоставлен</h2><p style="font-family:system-ui;text-align:center;color:#777">Ваш Telegram ID <b>${String(tg.sub||'')}</b> ещё не добавлен в администрацию.</p>`);
    res.redirect('/');
  }catch(e){ console.error('[TELEGRAM AUTH]',e); res.status(500).send('Ошибка авторизации через Telegram: '+String(e.message||e)); }
});
app.get('/login',(req,res)=>res.send(`<!doctype html><html lang="ru"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>SerkovTools — Telegram</title><style>body{margin:0;min-height:100vh;display:grid;place-items:center;background:radial-gradient(circle at 20% 10%,#229ed955,transparent 35%),#080611;color:#fff;font-family:Inter,system-ui}.card{width:min(420px,calc(100% - 32px));padding:34px;border:1px solid #ffffff18;border-radius:28px;background:linear-gradient(145deg,#151d25ee,#0b0813ee);box-shadow:0 30px 90px #0008;text-align:center}.logo{font-size:42px;margin-bottom:12px}.muted{color:#a99fba;line-height:1.6}.tg{display:block;margin-top:24px;padding:15px;border-radius:16px;text-decoration:none;color:#fff;font-weight:800;background:linear-gradient(135deg,#229ed9,#36b8f2);box-shadow:0 14px 34px #229ed933}.foot{margin-top:16px;color:#756b8e;font-size:12px}</style></head><body><main class="card"><div class="logo">✈️</div><h1>SerkovTools</h1><p class="muted">Закрытая панель администрации.<br>Вход выполняется через Telegram.</p><a class="tg" href="/auth/telegram">Войти через Telegram</a><div class="foot">Доступ получают только Telegram-профили, заранее добавленные в администрацию.</div></main></body></html>`));
app.get('/logout', async (req,res)=>{ await logAction(req.session?.telegramId||'unknown','LOGOUT','Выход из панели',getClientIp(req)); req.session.destroy(()=>res.redirect('/login')); });
...
async function requireAdmin(req,res,next){ if(req.session?.isAdmin && req.session.adminId)return next(); return res.redirect('/login'); }
...
app.get('/',requireAdmin,async(req,res)=>{
  ...
  const safe=JSON.stringify({id:admin.id,telegramId:admin.telegram_id||'',telegramUsername:admin.telegram_username||'',discordId:admin.discord_id||'',nickname:admin.nickname||'',position:admin.position||'',level:Number(admin.admin_level||admin.role_level||0),role:admin.role_name||'',avatar:admin.telegram_avatar||admin.vk_avatar||''}).replace(/</g,'\\u003c');
...
app.get('/api/admins',requireAdmin,async(req,res)=>{const rows=await db.all(`SELECT a.id,a.telegram_id,a.telegram_username,a.discord_id,a.nickname,a.position,a.admin_level,COALESCE(a.telegram_avatar,a.vk_avatar) AS avatar FROM web_admins a ORDER BY a.admin_level DESC,a.nickname COLLATE NOCASE ASC`);res.json({admins:rows,stats:bridgeStats?.adminMessages||{},week:bridgeStats?.week||''});});
app.post('/api/admins',requirePermission('manage_admins'),async(req,res)=>{const {telegram_id,discord_id,position,appointment_reason,admin_level}=req.body;const level=Number(admin_level);if(!/^\\d+$/.test(String(telegram_id||''))||!/^\\d+$/.test(String(discord_id||'')))return res.status(400).json({error:'Telegram ID и Discord ID должны быть числами'});if(level<1||level>8)return res.status(400).json({error:'Максимальный уровень — 8'});if(!position||!appointment_reason)return res.status(400).json({error:'Заполните должность и причину'});const exists=await db.get(`SELECT id FROM web_admins WHERE telegram_id=? OR discord_id=?`,[String(telegram_id),String(discord_id)]);if(exists)return res.status(400).json({error:'Такой Telegram ID или Discord ID уже зарегистрирован'});if(level>=Number(req.admin.admin_level||0))return res.status(403).json({error:'Нельзя создать администратора равного или выше вашего уровня'});let tgName='TG '+String(telegram_id),tgAvatar='';const role=await db.get(`SELECT id FROM web_roles WHERE level=? ORDER BY id LIMIT 1`,[level]);await db.run(`INSERT INTO web_admins (username,password_hash,role_id,telegram_id,telegram_username,telegram_avatar,discord_id,nickname,position,appointment_reason,admin_level) VALUES (?,?,?,?,?,?,?,?,?,?)`,[`tg_${telegram_id}`,'',role?.id||null,String(telegram_id),'',tgAvatar,String(discord_id),tgName,String(position).trim(),String(appointment_reason).trim(),level]);await logAction(req.session.telegramId,'CREATE_ADMIN',`Создан администратор Telegram ${telegram_id}`,getClientIp(req));res.json({ok:true});});
app.delete('/api/admins/:id',requirePermission('manage_admins'),async(req,res)=>{const row=await db.get(`SELECT * FROM web_admins WHERE id=?`,[req.params.id]);if(!row)return res.status(404).json({error:'Профиль не найден'});if(Number(row.admin_level||0)>=Number(req.admin.admin_level||0))return res.status(403).json({error:'Нельзя удалить администратора равного или более высокого уровня'});if(Number(row.id)===Number(req.admin.id))return res.status(400).json({error:'Нельзя удалить себя'});await db.run(`DELETE FROM web_admins WHERE id=?`,[req.params.id]);await logAction(req.session.telegramId,'DELETE_ADMIN',`Удалён профиль ID ${req.params.id}`,getClientIp(req));res.json({ok:true});});
