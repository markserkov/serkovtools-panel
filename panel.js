...
if (!mainAdmin) {
  ...
  await db.run(`INSERT INTO web_admins (username,password_hash,role_id,nickname,position,admin_level) VALUES (?,?,?,?,?,?)`, [ADMIN_USERNAME,hash,ownerRole?.id||null,ADMIN_USERNAME,'Владелец',8]);
  console.log('[WEB] Главный админ создан');
} else if (!mainAdmin.password_hash) {
  const hash = await bcrypt.hash(ADMIN_PASSWORD, 12);
  await db.run(`UPDATE web_admins SET password_hash=?, admin_level=8 WHERE id=?`, [hash,mainAdmin.id]);
  console.log('[WEB] Пароль главного админа восстановлен из ADMIN_PASSWORD');
}
async function getAdminWithRole(username){ return db.get(`SELECT a.*,r.name AS role_name,r.level AS role_level,r.permissions FROM web_admins a LEFT JOIN web_roles r ON a.role_id=r.id WHERE a.username=?`,[username]); }
async function logLogin(username,ip,userAgent,success){ try{await db.run(`INSERT INTO web_logins (username,ip,user_agent,success) VALUES (?,?,?,?)`,[username,ip,userAgent,success?1:0]);}catch{} }
app.get('/login',(req,res)=>{
  if(req.session?.isAdmin)return res.redirect('/');
  res.send(`<!doctype html><html lang="ru"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Вход — SerkovTools</title><style>body{margin:0;min-height:100vh;display:grid;place-items:center;background:radial-gradient(circle at 20% 10%,#43208455,transparent 34%),radial-gradient(circle at 100% 30%,#0b5fff22,transparent 30%),#070711;color:#fff;font-family:Inter,system-ui}.card{width:min(420px,calc(100% - 32px));padding:34px;border:1px solid #ffffff18;border-radius:28px;background:linear-gradient(145deg,#21153bd9,#0b0a13ee);box-shadow:0 30px 90px #0008}.logo{font-size:42px}.muted{color:#a39bb7;line-height:1.6}.field{margin:14px 0}.field label{display:block;font-size:11px;color:#bdb4cb;margin-bottom:7px}.field input{width:100%;box-sizing:border-box;padding:14px;border-radius:14px;border:1px solid #ffffff14;background:#090811;color:#fff;outline:none}.field input:focus{border-color:#6b8cff;box-shadow:0 0 0 4px #6b8cff18}.btn{width:100%;padding:14px;border:0;border-radius:14px;color:#fff;font-weight:800;cursor:pointer;background:linear-gradient(135deg,#1676ff,#8146ef);margin-top:8px}.foot{margin-top:18px;color:#756b8e;font-size:11px;text-align:center}</style></head><body><main class="card"><div class="logo">⚡</div><h1>SerkovTools</h1><p class="muted">Закрытая панель администрации.<br>Войдите по логину и паролю.</p><form method="POST" action="/login"><div class="field"><label>ЛОГИН</label><input name="username" required autocomplete="username" placeholder="Введите логин"></div><div class="field"><label>ПАРОЛЬ</label><input name="password" type="password" required autocomplete="current-password" placeholder="Введите пароль"></div><button class="btn" type="submit">Войти в панель →</button></form><div class="foot">Доступ получают только созданные администраторы.</div></main></body></html>`);
});
app.post('/login',async(req,res)=>{
  const username=String(req.body.username||'').trim(),password=String(req.body.password||''),ip=getClientIp(req),ua=req.headers['user-agent']||'';
  const admin=await getAdminWithRole(username);
  if(admin&&admin.password_hash&&await bcrypt.compare(password,admin.password_hash)){req.session.isAdmin=true;req.session.adminId=admin.id;req.session.username=admin.username;await logLogin(username,ip,ua,true);await logAction(username,'LOGIN_SUCCESS','Успешный вход',ip);return res.redirect('/');}
  await logLogin(username||'unknown',ip,ua,false);await logAction(username||'unknown','LOGIN_FAIL','Неудачная попытка входа',ip);
  res.status(401).send(`<h2 style="font-family:system-ui;color:#ef4444;text-align:center;margin-top:100px">Неверный логин или пароль</h2><p style="font-family:system-ui;text-align:center"><a href="/login">Попробовать снова</a></p>`);
});
...
app.get('/',requireAdmin,async(req,res)=>{
  ...
  const safe=JSON.stringify({id:admin.id,username:admin.username||'',vkId:admin.vk_id||'',discordId:admin.discord_id||'',nickname:admin.nickname||'',position:admin.position||'',level:Number(admin.admin_level||admin.role_level||0),role:admin.role_name||'',avatar:admin.vk_avatar||''}).replace(/</g,'\\u003c');
...
app.get('/api/admins',requireAdmin,async(req,res)=>{const rows=await db.all(`SELECT a.id,a.username,a.vk_id,a.discord_id,a.nickname,a.position,a.admin_level,a.vk_avatar AS avatar FROM web_admins a ORDER BY a.admin_level DESC,a.nickname COLLATE NOCASE ASC`);res.json({admins:rows,stats:bridgeStats?.adminMessages||{},week:bridgeStats?.week||''});});
app.post('/api/admins',requirePermission('manage_admins'),async(req,res)=>{
  const {username,password,vk_id,discord_id,position,appointment_reason,admin_level}=req.body; const level=Number(admin_level);
  if(!/^[A-Za-z0-9_.-]{3,32}$/.test(String(username||'')))return res.status(400).json({error:'Логин: 3–32 символа, только латиница, цифры, _, ., -'});
  if(String(password||'').length<6)return res.status(400).json({error:'Пароль должен содержать минимум 6 символов'});
  if(!/^\d+$/.test(String(vk_id||''))||!/^\d+$/.test(String(discord_id||'')))return res.status(400).json({error:'VK ID и Discord ID должны быть числами'});
  if(level<1||level>8)return res.status(400).json({error:'Максимальный уровень — 8'});
  if(!position||!appointment_reason)return res.status(400).json({error:'Заполните должность и причину'});
  if(level>=Number(req.admin.admin_level||req.admin.role_level||0))return res.status(403).json({error:'Нельзя создать администратора равного или выше вашего уровня'});
  const exists=await db.get(`SELECT id FROM web_admins WHERE username=? OR vk_id=? OR discord_id=?`,[String(username).trim(),String(vk_id),String(discord_id)]);
  if(exists)return res.status(400).json({error:'Такой логин, VK ID или Discord ID уже зарегистрирован'});
  let vkName='VK '+String(vk_id),avatar='';
  try{const tok=process.env.VK_SERVICE_TOKEN||'';if(tok){const u=await fetch('https://api.vk.com/method/users.get?'+new URLSearchParams({user_ids:String(vk_id),fields:'photo_200',access_token:tok,v:'5.199'})).then(r=>r.json());const x=u.response?.[0];if(x){vkName=`${x.first_name||''} ${x.last_name||''}`.trim()||vkName;avatar=x.photo_200||'';}}}catch{}
  const hash=await bcrypt.hash(String(password),12); const role=await db.get(`SELECT id FROM web_roles WHERE level=? ORDER BY id LIMIT 1`,[level]);
  await db.run(`INSERT INTO web_admins (username,password_hash,role_id,vk_id,discord_id,nickname,position,appointment_reason,vk_avatar,admin_level) VALUES (?,?,?,?,?,?,?,?,?,?)`,[String(username).trim(),hash,role?.id||null,String(vk_id),String(discord_id),vkName,String(position).trim(),String(appointment_reason).trim(),avatar,level]);
  await logAction(req.admin.username,'CREATE_ADMIN',`Создан администратор ${username} / VK ${vk_id}`,getClientIp(req)); res.json({ok:true});
});
app.delete('/api/admins/:id' ,requirePermission('manage_admins'),async(req,res)=>{const row=await db.get(`SELECT * FROM web_admins WHERE id=?`,[req.params.id]);if(!row)return res.status(404).json({error:'Профиль не найден'});if(Number(row.admin_level||0)>=Number(req.admin.admin_level||0))return res.status(403).json({error:'Нельзя удалить администратора равного или более высокого уровня'});if(Number(row.id)===Number(req.admin.id))return res.status(400).json({error:'Нельзя удалить себя'});await db.run(`DELETE FROM web_admins WHERE id=?`,[req.params.id]);await logAction(req.session.username,'DELETE_ADMIN',`Удалён профиль ID ${req.params.id}`,getClientIp(req));res.json({ok:true});});
